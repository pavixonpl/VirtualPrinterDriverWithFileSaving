...
namespace PrinterDriverApp
{
    public class noweTestyDrivera
    {
        ...
        private const string PRINTERNAME = "IcPenPDF Two";
        private const string DRIVERNAME = "IcPenPDF Virtual Printer";
        private const string HARDWAREID = "ICPEN_Driver";
        private const string PORTMONITOR = "ICPEN Two";
        private const string MONITORDLL = "clawmon.dll";
        private const string MONITORUIDLL = "clawmonui.dll";
        private const string PORTNAME = "ICPEN:";
        ...
        public noweTestyDrivera()
        {
            string clawmonpath;
            if (Environment.Is64BitOperatingSystem)
            {
                clawmonpath = Path.GetDirectoryName(AppContext.BaseDirectory) + @"\x64\";
            }
            else
            {
                clawmonpath = Path.GetDirectoryName(AppContext.BaseDirectory) + @"\x86\";
            }
            InstallclawPDFPrinter(clawmonpath, AppContext.BaseDirectory + @"\ProjektNaSzybko.exe");
        }
                ...
                if (CopyPrinterDriverFiles(driverSourceDirectory, printerDriverFiles.Concat(printerDriverDependentFiles).ToArray()))
                {
                    ...
                    if (AddclawPDFPort())
                    {
                        ...
                        if (InstallclawPDFPrinterDriver())
                        {
                            ...
                            if (AddCustomclawPDFPrinter(PRINTERNAME))
                            {
        ...
        private bool ConfigureclawPDFPort(String commandValue)
        {
            ...
            try
            {
                ...
                portConfiguration.SetValue("ExecPath", Directory.GetCurrentDirectory(), RegistryValueKind.String);
                ...
                portConfiguration.SetValue("UserCommand", commandValue, RegistryValueKind.String);
